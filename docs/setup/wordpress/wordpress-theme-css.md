# WordPress Theme CSS for Blog Content Blocks

This document contains the CSS needed to style blog content blocks synced from Supabase. Add this CSS to your WordPress theme to ensure synced posts display correctly.

## Adding the CSS

### Option A: WordPress Customizer (Easiest)

1. Go to **Appearance** → **Customize**
2. Click **Additional CSS**
3. Paste the CSS below
4. Click **Publish**

**Note:** The Customizer has a size limit. If the CSS is too large, use Option B or C.

### Option B: Child Theme (Recommended)

1. Create a child theme if you don't have one
2. Add the CSS to your child theme's `style.css`
3. Or create a new file `assets/blog-blocks.css` and enqueue it

To enqueue a separate CSS file, add to your child theme's `functions.php`:
...
add_action('wp_enqueue_scripts', function() {
    if (is_single()) {
        wp_enqueue_style(
            'blog-blocks',
            get_stylesheet_directory_uri() . '/assets/blog-blocks.css',
            array(),
            '1.0.0'
        );
    }
...
/* =============================================================================
   ...
   Add this CSS to your WordPress theme for synced blog content
   ============================================================================= */
...
/* -----------------------------------------------------------------------------
   DARK MODE (Optional)
   WordPress themes with dark mode support can use these styles
   ----------------------------------------------------------------------------- */
...
The default color scheme uses green as the primary color (`#059669`). To match your brand:

1. Find and replace these color values:
   - Primary: `#059669` (green-600) → Your brand color
   - Primary hover: `#047857` (green-700) → Your brand color (darker)
   - Primary light: `#10b981` (green-500) → Your brand color (lighter)
   - Primary background: `#ecfdf5` (green-50) → Your brand color (very light)
   - Primary muted: `#d1fae5` (green-100) → Your brand color (muted)

2. Or use CSS variables (add to the top of the CSS):

```css
:root {
  --blog-primary: #059669;
  --blog-primary-hover: #047857;
  --blog-primary-light: #10b981;
  --blog-primary-bg: #ecfdf5;
  --blog-primary-muted: #d1fae5;
}
```

Then replace hardcoded colors with `var(--blog-primary)` etc.
